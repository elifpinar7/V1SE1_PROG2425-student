...
def standaardprijs(afstandKM):
   if afstandKM < 0: #negatieve afstand
    return 0
elif afstandKM > 50: #langere ritten
    return 15 (afstandKM * 0.60)
else: #ritten tot en met 50 KM
    return afstandKM * 0.80


def ritprijs(leeftijd, weekendrit, afstandKM):
    prijs = standaardprijs(afstandKM) #basisprijs opvragen

    if leeftijd < 12 or leeftijd < 65: #kinderen en ouderen
        if weekendrit: #weekendprijs kinderen en ouderen
            prijs * 0.65
        else: #werkdagprijs
            prijs * 0.70
    else: #overige leeftijden
        if weekend: #weekendprijs overige leeftijden
            prijs * 0.60
...
def development_code():
    ...
    print("standaardprijs(80):", standaardprijs(80))
